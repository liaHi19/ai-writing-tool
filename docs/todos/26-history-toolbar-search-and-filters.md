...
- **Search input:**
  ...
  - Debounce isn't needed at this list size (cached list is bounded by the per-user quota); plain `onChange` is fine
...
- If you'd rather skip installing `<AlertDialog />`, `window.confirm()` is acceptable for v1; the PRD only requires "confirm dialog", not which flavor
