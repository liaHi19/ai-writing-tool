...
import { GenerationDialog } from "@/components/shared/generation-dialog";
    ...
    <>
      <article className="group relative flex min-h-55 flex-col gap-3 rounded-(--radius) border bg-(--surface) p-4.5 transition-colors hover:border-(--fg-muted)">
        ...
        <div
          ...
          onKeyDown={(e) => {
            // only swallow the keys the card itself acts on
      ...
      <GenerationDialog gen={gen} open={open} onOpenChange={setOpen} />
    </>
  );
}
